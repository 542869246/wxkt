...
//					$("#"+my.fileId).tips({
//						side:2,
//			            msg:'图片文件超过10MB,不允许上传!',
//			            bg:'#AE81FF',
//			            time:5
//			        });
//					return false;
//				}
			    var FileController = my.getBasePath()+"/pictures/upload.do?"+new Date().getTime(); // 接收上传文件的后台地址   
...
//			    	my.progressFunction(e);
			    	...
			    	if(bfb!=100){
			    		$("#"+my.fileId).siblings(".rate").show().find("p").css("width",bfb+"%");
			    	}else{
			    	}
			    }, false);  
...
//					        });
	...
	.attr({ rel: "stylesheet",
	type: "text/css",
	href: "css/ajaxfile.css?"+new Date().getTime()
	})
	.appendTo("head");
} catch (e) {
	alert("文件上传出错");
