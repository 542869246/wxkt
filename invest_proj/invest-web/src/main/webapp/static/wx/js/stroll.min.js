...
(function() {
	...
	function l(n, m) {
		if(!n.nodeName || /^(ul|li)$/i.test(n.nodeName) === false) {
			return false;
		} else {
			if(e(n)) {
				g(n);
		...
		var o = b ? new c(n) : new k(n);
		if(m && m.live) {
			o.syncInterval = setInterval(function() {
				o.sync.call(o);
			}, h);
		}
		o.sync();
	...
	function f(q, r, o) {
		...
		if(typeof q === "string") {
			...
			if(typeof q === "object" && typeof q.length === "number") {
	...
	k.prototype.sync = function() {
		...
		for(var n = 0, m = this.items.length; n < m; n++) {
			var o = this.items[n];
			o._offsetHeight = o.offsetHeight;
			o._offsetTop = o.offsetTop;
			o._offsetBottom = o._offsetTop + o._offsetHeight;
			o._state = "";
		}
		this.update(true);
	};
	k.prototype.update = function(q) {
		var r = this.element.pageYOffset || this.element.scrollTop,
			p = r + this.listHeight;
		if(r !== this.lastTop || q) {
			this.lastTop = r;
			for(var n = 0, m = this.items.length; n < m; n++) {
				var o = this.items[n];
				if(o._offsetBottom < r) {
					if(o._state !== "past") {
						o._state = "past";
						o.classList.add("past");
	...
	c.prototype.sync = function() {
		...
		this.top.max = o._offsetBottom - this.listHeight;
		this.update(true);
		this.bind();
	};
	c.prototype.bind = function() {
		var m = this;
		this.touchStartDelegate = function(n) {
			m.onTouchStart(n);
		};
		this.touchMoveDelegate = function(n) {
			m.onTouchMove(n);
		};
		this.touchEndDelegate = function(n) {
			m.onTouchEnd(n);
		};
	...
	c.prototype.onTouchStart = function(n) {
		...
		if(n.touches.length === 1) {
			...
			this.touch.start = n.touches[0].clientY;
	...
	c.prototype.onTouchMove = function(n) {
		if(n.touches.length === 1) {
			var m = this.touch.value;
			this.touch.value = n.touches[0].clientY;
			this.touch.lastMove = Date.now();
			var o = (this.touch.value > this.touch.previous && this.velocity < 0) || (this.touch.value < this.touch.previous && this.velocity > 0);
			if(this.touch.isAccellerating && o) {
			...
			this.touch.previous = m;
		}
	};
	c.prototype.onTouchEnd = function(n) {
		var m = this.touch.start - this.touch.value;
		...
		this.touch.isAccellerating = false;
		clearInterval(this.touch.accellerateTimeout);
		if(Math.abs(this.velocity) > 4 || Math.abs(m) > 10) {
			n.preventDefault();
		}
	};
	c.prototype.update = function(q) {
		var r = this.top.value + this.velocity + this.touch.offset;
		if(this.velocity || this.touch.offset) {
			this.element.scrollTop = r;
			r = Math.max(0, Math.min(this.element.scrollTop, this.top.max));
			this.top.value = r - this.touch.offset;
		}
		...
		if(r !== this.top.natural || q) {
			this.top.natural = r;
			this.top.value = r - this.touch.offset;
			var p = r + this.listHeight;
			for(var n = 0, m = this.items.length; n < m; n++) {
				var o = this.items[n];
				if(o._offsetBottom < r) {
					if(this.velocity <= 0 && o._state !== "past") {
						o.classList.add("past");
						o._state = "past";
					}
				} else {
					if(o._offsetTop > p) {
						if(this.velocity >= 0 && o._state !== "future") {
							o.classList.add("future");
							o._state = "future";
						}
					} else {
	...
	c.prototype.destroy = function() {
		k.prototype.destroy.apply(this);
