...
public class tls_sigature {
	...
	public String getToken(int sdkId,String identifier,PageData pd){
		try{			
			//Use pemfile keys to test
		    String privStr=pd.getString("PRIVATE_KEY");
		    String start="-----BEGIN PRIVATE KEY-----";
		    String end="-----END PRIVATE KEY-----";
		    String[] str=privStr.split(" ");
		    String newPrivStr2=start+"\n";
		   ...
		   newPrivStr2=newPrivStr2+"\n"+end;
			//change public pem string to public string
			//String pubStr = pd.getString("PUBLICKEY");
            // generate signature
            GenTLSSignatureResult result = GenTLSSignatureEx(sdkId, identifier, newPrivStr2);
          ...
          /*  // check signature
            CheckTLSSignatureResult checkResult = CheckTLSSignatureEx(result.urlSig, sdkId, identifier, pubStr);
	...
	public  GenTLSSignatureResult GenTLSSignatureEx(
		...
		try {
		    ...
		    int compressBytesLength = compresser.deflate(compressBytes);
		    compresser.end();
		    byte[] array = Arrays.copyOfRange(compressBytes,0,compressBytesLength);
		    String userSig = new String(base64_url.base64EncodeUrl(array));
	...
	public static void main(String[] args) {
		...
		System.out.println(str1.equals(newStr));
