...
public class NewsController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd.put("NEWS_CREATEBY",user.getUSER_ID());           //创建人id
		Date date = new Date();
		SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		pd.put("NEWS_CREATETIME", sf.format(date));
		newsService.save(pd);
	...
	public ModelAndView list(Page page) throws Exception{
		...
		List<PageData> productList=productsinfoService.selectAllProducts();
		//关键词检索条件
		...
		String productid=pd.getString("NEWS_PRODUCTS_ID");
		...
		if(null!=productid && "".equals(productid)){
			pd.put("NEWS_PRODUCTS_ID", productid);
		}
		page.setPd(pd);
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).getString("PRODUCTS_NAME"));	    //1
			vpd.put("var2", varOList.get(i).getString("NEWS_URL"));	    //2
			vpd.put("var3", varOList.get(i).getString("NEWS_TITLE"));	    //3
			vpd.put("var4", varOList.get(i).getString("NEWS_INSERTTIME"));	    //4
			vpd.put("var5", varOList.get(i).getString("NEWS_CREATEBY"));	    //5
			varList.add(vpd);
		}
