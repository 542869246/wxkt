...
public class Uploader {
	...
	public void upload() throws Exception {
		...
		String basePath = request.getScheme() + "://"
				...
				+ path + "/";
		this.UPLOAD_REQUESTPATH = basePath;
		...
		try {
			...
			this.fileName = this.getName(this.originalName);
			this.type = this.getFileExt(this.fileName);
			String fileSavePath = request.getRealPath("/")+this.UPLOAD_FOLDER_PATH+savePath;
			File file = new File(fileSavePath);
			if(!file.isDirectory()){
				file.mkdirs();
			}
			
			this.url = this.UPLOAD_FOLDER_PATH+savePath+ "/UE" +this.fileName;
			FileOutputStream fos = new FileOutputStream(fileSavePath+ "/UE" +this.fileName);
			BufferedInputStream bis = new BufferedInputStream(this.inputStream);
			byte[] buff = new byte[128];
			int count = -1;
