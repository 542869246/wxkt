				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<form action="webschedule/edit.do" name="Form" id="Form"
								method="post">
								...
								<div id="zhongxin" style="padding-top: 13px;">
									...
									<table id="table_report"
										...
										<td style="display: none;"><input type="text"value="${pd2.SCHEDULE_ID}" maxlength="100"
												name="SCHEDULE_ID" id="SCHEDULE_ID"
												style="width: 98%;" /></td>
										<tr>
											<td
												style="width: 75px; text-align: right; padding-top: 13px;"
												class="bitianxiang">学生:</td>
											<td><input type="text"
												value="${pd.stuName.STUDENT_NAME}" maxlength="32"
												readonly="readonly" placeholder="" title="学生"
												style="width: 98%;" /></td>
										</tr>
										<tr>
											<td
												style="width: 75px; text-align: right; padding-top: 13px;"
												class="bitianxiang">课后评价时间:</td>
											<td><input class="span10 date-picker"
												name="SCHEDULE_DATETIME" id="SCHEDULE_DATETIME"
												value="${pd2.SCHEDULE_DATETIME}" type="text"
												data-date-format="yyyy-mm-dd hh:mm:ss" readonly="readonly"
												placeholder="课后评价时间" title="课后评价时间" style="width: 98%;" /></td>
										</tr>

										<tr>
											<td
												style="width: 75px; text-align: right; padding-top: 13px;"
												class="bitianxiang">作业类型:</td>
											<td><select name="SCHEDULE_TASKTYPE"
												id="SCHEDULE_TASKTYPE"
												onchange='btnChange(this[selectedIndex].value,this[selectedIndex].text);'>
										...
										<c:if test="${pd.COURSES_ID==null || pd.COURSES_ID==''  }">
											<tr>
												<td
													style="width: 75px; text-align: right; padding-top: 13px;"
													class="bitianxiang">选择课程:</td>
												<td><select id="COURSES_ID" name="COURSES_ID"
													onchange="changecourse(this.value)">
														<option value="">--请选择--</option>
														<c:forEach items="${courseslist }" var="course">
															<option value="${course.COURSE_ID }">${course.COURSES_NAME }</option>
														</c:forEach>
												</select></td>
											</tr>
										</c:if>
								...
								<td>
									<select id="SUBJECT_ID" name="SUBJECT_ID">
										...
										<c:forEach items="${subjectList }" var="sub">
											<option value="${sub.SUBJECT_ID }" <c:if test="${sub.SUBJECT_ID==pd2.SUBJECT_ID }">selected</c:if>>${sub.SUBJECT_NAME }</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							<tr>
								...
								<td><input class="span10 date-picker1" name="ARRIVELEAVETIME" id="ARRIVELEAVETIME" value="${pd2.ARRIVELEAVETIME}" type="text" data-date-format="yyyy-mm-dd hh:mm:ss" readonly="readonly" placeholder="课后评价时间" title="课后评价时间" style="width:98%;"/></td>
							</tr>
							<tr id="TIMEDIFF">
							</tr>
							<tr id="CONTENT">
							...
							<tr id="stats">
								<td style="width:75px;text-align: right;padding-top: 13px;" class="bitianxiang">通过状态</td>
								<td>
								<select name="STATE" id="STATE">
										<option value="">请选择</option>
											<option value="0"<c:if test="${pd2.STATE==0}">selected="selected"</c:if>>通过</option>
											<option value="1"<c:if test="${pd2.STATE==1}">selected="selected"</c:if>>不通过</option>
									</select>
								</td>
							</tr>
							<tr>
	...
	<script type="text/javascript">
		...
		function btnChange(values,showName){
			...
			if(values=="zylr"){//作业录入
				...
				var option=`<td style="width:75px;text-align: right;padding-top: 13px;" class="bitianxiang">学习时长:</td>
					<td><input type="text" value="${pd2.SCHEDULE_TIMEDIFF}" maxlength="32"  placeholder="" title="学习时长" style="width:98%;" id="SCHEDULE_TIMEDIFF" name="SCHEDULE_TIMEDIFF"/></td>
				`
				var option1=`<td style="width:75px;text-align: right;padding-top: 13px;" class="bitianxiang">学习内容:</td>
					<td><input type="text" value="${pd2.SCHEDULE_CONTENT}" maxlength="32"  placeholder="" title="学习时长" style="width:98%;" id="SCHEDULE_CONTENT" name="SCHEDULE_CONTENT"/></td>
				`
				$("#TIMEDIFF").html(option);
				$("#CONTENT").html(option1);
