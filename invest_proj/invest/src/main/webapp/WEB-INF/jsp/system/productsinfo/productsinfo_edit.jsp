	...
	<SCRIPT type="text/javascript">
		...
		function onClick(e, treeId, treeNode) {
			...
			if(CurInput =='1'){
				...
				zTree= $.fn.zTree.getZTreeObj("treeInvestDemo");
			}
		...
		<!-- 保存下拉列表的值 -->
		function saveValue2(v){
			var cityObj = $("#investSel");
			var type = $("#PRODUCTS_TYPE2_ID");
			if(oldv1 != v ){
				type.attr("value", id);
				cityObj.attr("value", v);
				oldv1=v;
		...
		function showMenu(fn) {
			...
			if(fn=='1'){
				...
				CurInput = '2';
				var cityObj = $("#investSel");
				var cityOffset = $("#investSel").offset();
				$("#menuInvestContent").css({left:cityOffset.left + "px", top:cityOffset.top + cityObj.outerHeight() + "px"}).slideDown("fast");
		...
		//隐藏投资分组树状图
		function hideMenu2() {
			$("#menuInvestContent").fadeOut("fast");
			$("body").unbind("mousedown", onBodyDown);
		}
		function onBodyDown2(event) {
			if (!(event.target.id == "menuInvestContent" || $(event.target).parents("#menuInvestContent").length>0)) {
				hideMenu2();
			}
		}
		...
		//回显产品分组
		function huiXian1(treeObj){
				var nodes = treeObj.getNodes();
				var node = treeObj.getNodeByParam("id", "${pd.GROUPBY_USER_ID}");//根据ID获取节点
				treeObj.selectNode(node);//显示选中节点
				$("#attrSel").val(node.name);//给类型赋值
				$("#ATTR_TYPEID").val(node.id);
				
		}
		//回显投资分组
		function huiXian2(treeObj){
				var nodes = treeObj.getNodes();
				var node = treeObj.getNodeByParam("id", "${pd.PRODUCTS_TYPE2_ID}");//根据ID获取节点
				console.info(node);
				treeObj.selectNode(node);//显示选中节点
				$("#investSel").val(node.name);//给类型赋值
				$("#PRODUCTS_TYPE2_ID").val(node.id);
		...
		$(document).ready(function(){
			...
			//加载产品类型树状图
			var zn = '${zTreeInvestNodes}';
			var zTreeInvestNodes = eval(zn);
			$.fn.zTree.init($("#treeInvestDemo"), setting, zTreeInvestNodes);
			
			//修改时默认显示属性的类型
			if( ${msg eq 'edit'}){
				var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
				huiXian(treeObj);//调用回显方法
				var treeAttrDemo = $.fn.zTree.getZTreeObj("treeAttrDemo");
				huiXian1(treeAttrDemo);//调用回显方法
				var treeInvestDemo = $.fn.zTree.getZTreeObj("treeInvestDemo");
				huiXian2(treeInvestDemo);//调用回显方法
			}
		});
		...
		<div class="main-content-inner">
			<div class="page-content">
				<div class="row">
					...
					<!-- 产品类型下拉树 -->
					<div id="menuContent" class="menuContent" style="width:133px;display:none; position: absolute;z-index:999;background-color:#fff;" >
						<ul id="treeDemo" class="ztree" style="margin-top:0; width:133px;"></ul>
					</div>
					<!-- 产品属性下拉树 -->
					<div id="menuAttrContent" class="menuAttrContent" style="width:133px;display:none; position: absolute;z-index:99;background-color:#fff;" >
						<ul id="treeAttrDemo" class="ztree" style="margin-top:0; width:133px;"></ul>
					</div>
					<!-- 产品属性下拉树 -->
					<div id="menuInvestContent" class="menuInvestContent" style="width:133px;display:none; position: absolute;z-index:9;background-color:#fff;" >
						<ul id="treeInvestDemo" class="ztree" style="margin-top:0; width:133px;"></ul>
					</div>
					<form action="productsinfo/${msg }.do" name="Form" id="Form" method="post" >
						<input type="hidden" name="PRODUCTS_ZEXIANTU" id="PRODUCTS_ZEXIANTU"/>
						...
						<input type="hidden" name="PRODUCTS_TYPE2_ID" id="PRODUCTS_TYPE2_ID" value="${pd.PRODUCTS_TYPE2_ID}"/>
						<input type="hidden" name="ATTR_TYPEID" id="ATTR_TYPEID" value="${pd.GROUPBY_USER_ID}"/>
						<input type="hidden" name="GROUPBY_ID" id="GROUPBY_ID" value="${GROUPBY_ID}"/>
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<div class="nav-search">
											&nbsp;&nbsp;<input id="citySel" type="text" placeholder="点击选择类型" readonly="readonly"  class="nav-search-input"  autocomplete="off" name="attrType"  style="width:20%;"  onclick="showMenu(1);return false;"/>
							...
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">产品分组:</td>
								<td style="vertical-align:top;padding-left:2px;">
								 	<div class="nav-search">
											&nbsp;&nbsp;<input id="attrSel" type="text" placeholder="点击选择类型"  class="nav-search-input" readonly="readonly"  autocomplete="off" name="attrSel"   style="width:20%;"  onclick="showMenu(0);return false;"/>
							...
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">投资分组:</td>
								<td style="vertical-align:top;padding-left:2px;">
								 	<div class="nav-search">
											&nbsp;&nbsp;<input id="investSel" type="text" placeholder="点击选择类型"  class="nav-search-input" readonly="readonly"  autocomplete="off" name="investSel"   style="width:20%;"  onclick="showMenu(2);return false;"/>
											<a onclick="document.getElementById('investSel').value='';document.getElementById('PRODUCTS_TYPE2_ID').value='';" style="cursor:pointer;">清空</a>
										</span>
									</div>
								</td>
							</tr> 
							...
							<tr>
								...
								<td>
									...
									<div id="zjt">
										<c:if test="${not empty pd.PRODUCTS_ZEXIANTU}">
											<img src="<%=basePath%>${pd.PRODUCTS_ZEXIANTU}" class="center-block" width="100" height="100" onerror="javascript:this.src='static/images/404error.jpg';this.onerror=null"  />
											 <input type="hidden" name="PRODUCTS_ZEXIANTU" id="PRODUCTS_ZEXIANTU" value="${pd.PRODUCTS_ZEXIANTU}"/>
										</c:if>
									</div>
									
									<%-- <input type="file" id="IMG_URL" name="uploadFile"/>
									<%=basePath%>${pd.PRODUCTS_ZEXIANTU }
									<img alt="折线图" src="<%=basePath%>${pd.PRODUCTS_ZEXIANTU }"> --%>
							...
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">投资起点:</td>
...
<c:if test="${pd.PRODUCTSINFO_ID!=null}">
	<div style="height: 29px;width: 98%;margin:0px auto;border-bottom: 1px solid #4F99C6;padding-left:5px;">
		...
		<button class="mytab" 
			onclick="mytab.active(this,mytab.productZiliao);">${pd.PRODUCT_MENU==null?'详细资料':pd.PRODUCT_MENU}</button>
		<button class="mytab" 
			onclick="mytab.active(this,mytab.productNews);">${pd.PRODUCT_MENU==null?'相关新闻':pd.ROOM_MENU}</button>
			<button class="mytab" 
			onclick="mytab.active(this,mytab.productAttr);">${pd.PRODUCT_MENU==null?'基本要素':pd.ROOM_MENU}</button>
			<button class="mytab" 
...
<!-- /.main-container -->
	...
	<script type="text/javascript" src="plugins/imguploader/js/uploadProduct.js"></script>
	<!-- 页面底部js¨ -->
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function save(){
			...
			//判断上传图片类型是否为png/jpg
			/* var file=$("#PRODUCTS_ZEXIANTU")[0].value;
			//alert(file);
	...
	//页面tab初始化
		$(document).ready(function(){
			$(".active").click();
			alert("<%=basePath%>/information/list");
		});
		var mytab = {
